...
- `/time` — часы, секундомер, помодоро и настройки;
- `/time#clock`, `/time#stopwatch`, `/time#pomodoro`, `/time#settings` — прямые ссылки на режимы приложения времени.

Старые пути `/time.html` и `/index.html` продолжают работать. При открытии `/time` без hash по умолчанию активен режим `clock`. Hash сохраняет приоритет и позволяет сразу открыть любой другой режим.
...
- в dev переписывает `/` на `/src/pages/index.html`, а чистые и `.html`-маршруты на соответствующие Vite entry-файлы;
- в preview переписывает публичные маршруты на собранные `.html`-файлы.
...
В dev нужно проверить `/`, `/time`, `/note`, `/note-admin`, `/note-view` и их `.html`-варианты, а для страницы времени — четыре hash-режима. После build `npm run smoke` проверяет все публичные маршруты, локальные `href/src`, существование ассетов и их MIME.
